  ...
  "node_modules/@actions/core/lib/summary.js"(exports) {
    ...
    var Summary = class {
      ...
      addDetails(label, content3) {
        const element = this.wrap("details", this.wrap("summary", label) + content3);
        return this.addRaw(element).addEOL();
      }
      ...
      addHeading(text5, level) {
        const tag = `h${level}`;
        const allowedTag = ["h1", "h2", "h3", "h4", "h5", "h6"].includes(tag) ? tag : "h1";
        const element = this.wrap(allowedTag, text5);
        return this.addRaw(element).addEOL();
      }
      addSeparator() {
      ...
      addQuote(text5, cite) {
        ...
        const element = this.wrap("blockquote", text5, attrs);
        return this.addRaw(element).addEOL();
      }
      addLink(text5, href) {
        const element = this.wrap("a", text5, { href });
        return this.addRaw(element).addEOL();
      }
    };
...
function containerFlow(parent, context, safeOptions) {
  ...
  while (++index2 < children2.length) {
    ...
    if (index2 < children2.length - 1) {
      results.push(tracker.move(between(child, children2[index2 + 1])));
    }
  }
  indexStack.pop();
  return results.join("");
...
function containerPhrasing(parent, context, safeOptions) {
  ...
  while (++index2 < children2.length) {
    ...
    if (index2 + 1 < children2.length) {
      let handle2 = context.handle.handlers[children2[index2 + 1].type];
      if (handle2 && handle2.peek)
        handle2 = handle2.peek;
      after = handle2 ? handle2(children2[index2 + 1], parent, context, {
        before: "",
        after: "",
        ...tracker.current()
      }).charAt(0) : "";
...
function stripMarkdown(options = {}) {
  ...
  if (keep.length === 0) {
    ...
    for (key in handlers) {
      if (!keep.includes(key)) {
        map3[key] = handlers[key];
      }
    }
    index2 = -1;
    while (++index2 < keep.length) {
      key = keep[index2];
      if (!own5.call(handlers, key)) {
        throw new Error(
  ...
  function one2(node) {
    ...
    let result = node;
    if (type in map3) {
      const handler = map3[type];
      if (handler)
        result = handler(result);
    }
    result = Array.isArray(result) ? all2(result) : result;
    if ("children" in result) {
      result.children = all2(result.children);
    }
    return result;
  }
  function all2(nodes) {
    ...
    while (++index3 < nodes.length) {
      const value = one2(nodes[index3]);
      if (Array.isArray(value)) {
        result.push(...value.flatMap((d) => one2(d)));
      } else {
        result.push(value);
      }
    }
...
function clean(values) {
  ...
  while (++index2 < values.length) {
    ...
    if (previous2 && value.type === previous2.type && "value" in value) {
      previous2.value += value.value;
    } else {
      result.push(value);
      previous2 = value;
    }
